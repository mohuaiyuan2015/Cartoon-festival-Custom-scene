        ...
        ConnectionManager.startReceiveAndConnect(mContent, ConnectionManager.TYPE_WIFI_AP, apc,
                new ConnectionStatusCallback() {
                    ...
                    public void onConnectionCompleted(int status) {
                        ...
                        Log.i("Javen","onConnectionCompleted status: " + status);
...
//                        mISceneV.getResult(message);
                ...
                new DataReceiveCallback() {//收到客户端数据回调
                    ...
                    public void onReceiveData(String s) {
                        ...
                        Log.i("Javen","onReceiveData() called with: " + "s = [" + s + "]");
                    }
                }, null);
    }
                ...
                case Constants.NET_MSG:
                    ...
                    if (connect == ConnectionStatus.WIFI_CONNECTED_SUCCESS) {
                        ...
                        if(TobotUtils.isEmploy()){//首次使用
                            mainActivity.onInitiate();
                            user.setUltr("1");
                        }else if (!isLoad){//加载失败
                            mainActivity.onInitiate();
                        }
                        user.setMobile(mConnectionEntity.getCustomData());
                        user.setUltrAP("1");
...
//        if (!TobotUtils.isEqual(UserDBManager.getManager().getCurrentUser().getMobile(),mConnectionEntity.getCustomData())) {
            ...
            OkHttpUtils.get()
                    .url(Constants.ROBOT_BOUND + uuid + "/" + SHA1.gen(Constants.identifying + uuid)
                            ...
                            + "/" + mConnectionEntity.getCustomData())
                    ...
                    .addParams("mobile", mConnectionEntity.getCustomData())//手机号
                    .build()
                    .execute(new StringCallback() {
                        ...
                        public void onError(Call call, Exception e, int id) {
                            ...
                            Log.i("Javen","绑定失败.................." + call+"... "+ bind);
                        }

                        @Override
                        public void onResponse(String response, int id) {
                            Log.i("Javen","绑定.................."+response+"..."+id);
                        }
                    });
//        }
    }
