...
//        }
        ...
        String []actionCodeTemp=getResources().getStringArray(R.array.actionCodeArray);
        String []kindTemp=getResources().getStringArray(R.array.kind);
        for (int i=0;i<questionTemp.length;i++){
            Map<String,String>map=new HashMap<>();

            map.put(Constants.QUESTION,questionTemp[i]);
            map.put(Constants.ANSWER,answerTemp[i]);
            map.put(Constants.ACTION_CODE,actionCodeTemp[i]);
            map.put(Constants.KIND,kindTemp[i]);
            dataList.add(map);
        }
        ...
        myAdapter.setOnItemClickListener(new MyAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View itemView, int position) {
                ...
                if (ConnectionInfoCollector.getBluetoothDeviceModelList().isEmpty()) {
                    ...
                    String wirteData="";
                    String answer=dataList.get(position).get(Constants.ANSWER);
                    String actionCode=dataList.get(position).get(Constants.ACTION_CODE);
                    String kind=dataList.get(position).get(Constants.KIND);

                    wirteData+=kind+Constants.SEPARATOR_BETWEEN_KIND_AND_BYDY_TEXT;
                    ...
                    if (answer.length()<1){
                        ...
                        Log.d(TAG, "发送的数据: " + wirteData);
                        write(wirteData.getBytes());
                    }
                }else {
                    //mohuaiyuan 发送给多个蓝牙
